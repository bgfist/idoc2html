<script lang="ts">
    ...
    import { TargetPlatform } from '../src/generator';
    ...
    async function exportReplaceImage() {
        const { code: html, noImages } = await replaceHtmlImages(code, prefix);
        if (noImages) {
            makeToast('代码中没有图片!', { fontSize: '80px', border: 'success' });
        } else {
            makeToast('图片下载成功', { fontSize: '80px', border: 'success' });
        }
        dispatcher('close', [
            {
                code: html,
...
<div class="fixed left-0 right-0 top-0 bottom-0 bg-[hsla(0,0%,0%,0.6)] flex justify-center pt-63 items-start">
    <div class="bg-white rounded-16 p-20 pb-60 flex flex-col w-650 relative">
        ...
        {#if warn}
            <div>剪贴板内容跟生成的代码不一致，请确认是否只需导出部分代码</div>
        {/if}
        ...
        <div class="flex">
            图片尺寸:
            <select>
                <option>1倍图（PC默认采用1倍图）</option>
                <option>2倍图（h5默认采用2倍图）</option>
                <option>4倍图（原始预览图片）</option>
            </select>
        </div>
        <label class="cursor-pointer flex items-center w-200">
        ...
        <div>
            ...
            <label class="flex items-center my-10 text-16/28">
                ...
                <input type="text" class="flex-1 rounded-6 py-0 px-8 border text-16/28" />
            </label>
        </div>
